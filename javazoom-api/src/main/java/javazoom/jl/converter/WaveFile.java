...
public class WaveFile extends RiffFile {
	 ...
	 *
	 *
	 * public int OpenForRead (String Filename) { // Verify filename parameter as
	 * best we can... if (Filename == null) { return DDC_INVALID_CALL; } int retcode
	 * = Open ( Filename, RFM_READ );
	 * 
	 ...
	 * isn't standard PCM, so we don't know what it is! retcode = DDC_FILE_ERROR; }
	 * 
	 * if ( retcode == DDC_SUCCESS ) { pcm_data_offset = CurrentFilePosition();
	 * 
	 * // Figure out number of samples from // file size, current file position, and
	 * // WAVE header. retcode = Read (pcm_data, 8 ); num_samples =
	 * filelength(fileno(file)) - CurrentFilePosition(); num_samples /=
	 * NumChannels(); num_samples /= (BitsPerSample() / 8); } } } return retcode; }
	 */
	...
	*/
	public int OpenForWrite(String Filename, int SamplingRate, short BitsPerSample, short NumChannels) {
		...
		if (retcode == DDC_SUCCESS) {
			...
			if (retcode == DDC_SUCCESS) {
				...
				/*
				 * byte[] br = new byte[16]; br[0] = (byte) ((wave_format.data.wFormatTag >> 8)
				 * & 0x00FF); br[1] = (byte) (wave_format.data.wFormatTag & 0x00FF);
				 * 
				 * br[2] = (byte) ((wave_format.data.nChannels >> 8) & 0x00FF); br[3] = (byte)
				 * (wave_format.data.nChannels & 0x00FF);
				 * 
				 * br[4] = (byte) ((wave_format.data.nSamplesPerSec >> 24)& 0x000000FF); br[5] =
				 * (byte) ((wave_format.data.nSamplesPerSec >> 16)& 0x000000FF); br[6] = (byte)
				 * ((wave_format.data.nSamplesPerSec >> 8)& 0x000000FF); br[7] = (byte)
				 * (wave_format.data.nSamplesPerSec & 0x000000FF);
				 * 
				 * br[8] = (byte) ((wave_format.data.nAvgBytesPerSec>> 24)& 0x000000FF); br[9] =
				 * (byte) ((wave_format.data.nAvgBytesPerSec >> 16)& 0x000000FF); br[10] =
				 * (byte) ((wave_format.data.nAvgBytesPerSec >> 8)& 0x000000FF); br[11] = (byte)
				 * (wave_format.data.nAvgBytesPerSec & 0x000000FF);
				 * 
				 * br[12] = (byte) ((wave_format.data.nBlockAlign >> 8) & 0x00FF); br[13] =
				 * (byte) (wave_format.data.nBlockAlign & 0x00FF);
				 * 
				 * br[14] = (byte) ((wave_format.data.nBitsPerSample >> 8) & 0x00FF); br[15] =
				 * (byte) (wave_format.data.nBitsPerSample & 0x00FF); retcode = Write (br, 16);
				 */
	 ...
	 *
	 *
	 * public int WriteSample( short[] Sample ) { int retcode = DDC_SUCCESS; switch
	 * ( wave_format.data.nChannels ) { case 1: switch (
	 ...
	 * 
	 * case 2: switch ( wave_format.data.nBitsPerSample ) { case 8: retcode = Write
	 * ( Sample, 1 ); if ( retcode == DDC_SUCCESS ) { // &Sample[1] retcode = Write
	 * (Sample, 1 ); if ( retcode == DDC_SUCCESS ) { pcm_data.ckSize += 2; } }
	 * break;
	 * 
	 ...
	 * NumSamples() ) { return DDC_INVALID_CALL; } int SampleSize = (BitsPerSample()
	 * + 7) / 8; int rc = Seek ( pcm_data_offset + 8 + SampleSize * NumChannels() *
	 * SampleIndex ); return rc; }
	 */
	 ...
	 * Read 16-bit audio.
	 *
	 * public int ReadData (short[] data, int numData) {return super.Read ( data,
	 * numData * 2);}
	 */
	 ...
	 * Read 8-bit audio.
	 *
	 * public int ReadData ( byte[] data, int numData ) {return super.Read ( data,
	 * numData );}
	 */
	 ...
	 * int retcode = DDC_SUCCESS; switch ( wave_format.data.nBitsPerSample ) { case
	 * 8: retcode = Write ( LeftSample, 1 ); if ( retcode == DDC_SUCCESS ) { retcode
	 * = Write ( RightSample, 1 ); if ( retcode == DDC_SUCCESS ) { pcm_data.ckSize
	 * += 2; } } break;
	 * 
	 ...
	 * switch ( wave_format.data.nBitsPerSample ) { case 8: byte[] x = {0}; retcode
	 * = Read ( x, 1 ); Sample[0] = (short)(x[0]); break;
	 * 
	 ...
	 * ) { int retcode = DDC_SUCCESS; byte[] x = new byte[2]; short[] y = new
	 * short[2]; switch ( wave_format.data.nBitsPerSample ) { case 8: retcode = Read
	 * ( x, 2 ); L[0] = (short) ( x[0] ); R[0] = (short) ( x[1] ); break;
	 * 
	...
	*/
	...
	/*
	 * public int FourCC(String ChunkName) { byte[] p = {0x20,0x20,0x20,0x20};
	 * ChunkName.getBytes(0,4,p,0); int ret = (((p[0] << 24)& 0xFF000000) | ((p[1]
	 * << 16)&0x00FF0000) | ((p[2] << 8)&0x0000FF00) | (p[3]&0x000000FF)); return
	 * ret; }
	 */
