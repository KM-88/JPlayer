...
public class RiffFile {
			...
			case RFM_READ:
				try {
					...
					try {
						...
						file.read(br, 0, 8);
						fmode = RFM_READ;
						riff_header.ckID = ((br[0] << 24) & 0xFF000000) | ((br[1] << 16) & 0x00FF0000)
								| ((br[2] << 8) & 0x0000FF00) | (br[3] & 0x000000FF);
						riff_header.ckSize = ((br[4] << 24) & 0xFF000000) | ((br[5] << 16) & 0x00FF0000)
								| ((br[6] << 8) & 0x0000FF00) | (br[7] & 0x000000FF);
					} catch (IOException ioe) {
						file.close();
						fmode = RFM_UNKNOWN;
					}
	...
	public static int FourCC(String ChunkName) {
		...
		int ret = (((p[0] << 24) & 0xFF000000) | ((p[1] << 16) & 0x00FF0000) | ((p[2] << 8) & 0x0000FF00)
				| (p[3] & 0x000000FF));
		return ret;
	}
