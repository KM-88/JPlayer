...
public class WaveFileObuffer extends Obuffer {
	 ...
	 * 
	 * @param number_of_channels The number of channels of audio data this buffer
	 *                           will receive.
	 * 
	...
	public void write_buffer(int val) {
		 ...
		 * 
		 * //myBuffer[0] = (short)(((buffer[j]>>8)&0x000000FF) |
		 * ((buffer[j]<<8)&0x0000FF00)); //myBuffer[1] = (short)
		 * (((buffer[j+1]>>8)&0x000000FF) | ((buffer[j+1]<<8)&0x0000FF00)); myBuffer[0]
		 * = buffer[j]; myBuffer[1] = buffer[j+1]; rc = outWave.WriteData (myBuffer,2);
		 * }
		 */
	 ...
	 * thebuffer = null; int mode = maplay_args.MPEGheader.mode(); int
	 * which_channels = maplay_args.which_c; if (mode == Header.single_channel ||
	 * which_channels != MPEG_Args.both) thebuffer = new
	 * FileObuffer(1,maplay_args.output_filename); else thebuffer = new
	 * FileObuffer(2,maplay_args.output_filename); return(thebuffer); }
	 */
}
