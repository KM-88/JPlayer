...
final class SynthesisFilter {
	...
	/**
	 * Contructor. The scalefactor scales the calculated float pcm samples to short
	 * values (raw pcm samples are in [-1.0, 1.0], if no violations occur).
	 */
...
//	float[] pp = new float[16];
	...
	private void compute_pcm_samples0(Obuffer buffer) {
		...
		for (int i = 0; i < 32; i++) {
			...
			pcm_sample = ((vp[0 + dvp] * dp[0]) + (vp[15 + dvp] * dp[1]) + (vp[14 + dvp] * dp[2])
	...
	private void compute_pcm_samples1(Obuffer buffer) {
		...
		for (int i = 0; i < 32; i++) {
			...
			pcm_sample = ((vp[1 + dvp] * dp[0]) + (vp[0 + dvp] * dp[1]) + (vp[15 + dvp] * dp[2])
	...
	private void compute_pcm_samples2(Obuffer buffer) {
		...
		for (int i = 0; i < 32; i++) {
			...
			pcm_sample = ((vp[2 + dvp] * dp[0]) + (vp[1 + dvp] * dp[1]) + (vp[0 + dvp] * dp[2]) + (vp[15 + dvp] * dp[3])
	...
	private void compute_pcm_samples3(Obuffer buffer) {
		...
		for (int i = 0; i < 32; i++) {
			...
			pcm_sample = ((vp[3 + dvp] * dp[0]) + (vp[2 + dvp] * dp[1]) + (vp[1 + dvp] * dp[2]) + (vp[0 + dvp] * dp[3])
	...
	private void compute_pcm_samples4(Obuffer buffer) {
		...
		for (int i = 0; i < 32; i++) {
			...
			pcm_sample = ((vp[4 + dvp] * dp[0]) + (vp[3 + dvp] * dp[1]) + (vp[2 + dvp] * dp[2]) + (vp[1 + dvp] * dp[3])
	...
	private void compute_pcm_samples5(Obuffer buffer) {
		...
		for (int i = 0; i < 32; i++) {
			...
			pcm_sample = ((vp[5 + dvp] * dp[0]) + (vp[4 + dvp] * dp[1]) + (vp[3 + dvp] * dp[2]) + (vp[2 + dvp] * dp[3])
					+ (vp[1 + dvp] * dp[4]) + (vp[0 + dvp] * dp[5]) + (vp[15 + dvp] * dp[6]) + (vp[14 + dvp] * dp[7])
					+ (vp[13 + dvp] * dp[8]) + (vp[12 + dvp] * dp[9]) + (vp[11 + dvp] * dp[10])
	...
	private void compute_pcm_samples6(Obuffer buffer) {
		...
		for (int i = 0; i < 32; i++) {
			...
			pcm_sample = ((vp[6 + dvp] * dp[0]) + (vp[5 + dvp] * dp[1]) + (vp[4 + dvp] * dp[2]) + (vp[3 + dvp] * dp[3])
					+ (vp[2 + dvp] * dp[4]) + (vp[1 + dvp] * dp[5]) + (vp[0 + dvp] * dp[6]) + (vp[15 + dvp] * dp[7])
					+ (vp[14 + dvp] * dp[8]) + (vp[13 + dvp] * dp[9]) + (vp[12 + dvp] * dp[10])
	...
	private void compute_pcm_samples7(Obuffer buffer) {
		...
		for (int i = 0; i < 32; i++) {
			...
			pcm_sample = ((vp[7 + dvp] * dp[0]) + (vp[6 + dvp] * dp[1]) + (vp[5 + dvp] * dp[2]) + (vp[4 + dvp] * dp[3])
					+ (vp[3 + dvp] * dp[4]) + (vp[2 + dvp] * dp[5]) + (vp[1 + dvp] * dp[6]) + (vp[0 + dvp] * dp[7])
					+ (vp[15 + dvp] * dp[8]) + (vp[14 + dvp] * dp[9]) + (vp[13 + dvp] * dp[10])
	...
	private void compute_pcm_samples10(Obuffer buffer) {
		...
		for (int i = 0; i < 32; i++) {
			...
			pcm_sample = ((vp[10 + dvp] * dp[0]) + (vp[9 + dvp] * dp[1]) + (vp[8 + dvp] * dp[2]) + (vp[7 + dvp] * dp[3])
					+ (vp[6 + dvp] * dp[4]) + (vp[5 + dvp] * dp[5]) + (vp[4 + dvp] * dp[6]) + (vp[3 + dvp] * dp[7])
					+ (vp[2 + dvp] * dp[8]) + (vp[1 + dvp] * dp[9]) + (vp[0 + dvp] * dp[10]) + (vp[15 + dvp] * dp[11])
					+ (vp[14 + dvp] * dp[12]) + (vp[13 + dvp] * dp[13]) + (vp[12 + dvp] * dp[14])
	...
	private void compute_pcm_samples11(Obuffer buffer) {
		...
		for (int i = 0; i < 32; i++) {
			...
			pcm_sample = ((vp[11 + dvp] * dp[0]) + (vp[10 + dvp] * dp[1]) + (vp[9 + dvp] * dp[2])
	...
	private void compute_pcm_samples12(Obuffer buffer) {
		...
		for (int i = 0; i < 32; i++) {
			...
			pcm_sample = ((vp[12 + dvp] * dp[0]) + (vp[11 + dvp] * dp[1]) + (vp[10 + dvp] * dp[2])
	...
	private void compute_pcm_samples13(Obuffer buffer) {
		...
		for (int i = 0; i < 32; i++) {
			...
			pcm_sample = ((vp[13 + dvp] * dp[0]) + (vp[12 + dvp] * dp[1]) + (vp[11 + dvp] * dp[2])
	...
	private void compute_pcm_samples14(Obuffer buffer) {
		...
		for (int i = 0; i < 32; i++) {
			...
			pcm_sample = ((vp[14 + dvp] * dp[0]) + (vp[13 + dvp] * dp[1]) + (vp[12 + dvp] * dp[2])
	...
	private void compute_pcm_samples15(Obuffer buffer) {
		...
		for (int i = 0; i < 32; i++) {
			float pcm_sample;
			final float dp[] = d16[i];
			pcm_sample = ((vp[15 + dvp] * dp[0]) + (vp[14 + dvp] * dp[1]) + (vp[13 + dvp] * dp[2])
