...
public class PlayerApplet extends Applet implements Runnable {
	...
	/**
	 * Retrieves the <code>AudioDevice</code> instance that will be used to sound
	 * the audio data.
	 * 
	 * @return an audio device instance that will be used to sound the audio stream.
	 */
	 ...
	 * 
	 * @return an InputStream from which the MPEG audio data is read, or null if an
	 *         error occurs.
	 */
	...
	/**
	 * Starts this applet. An input stream and audio device are created and passed
	 * to the play() method.
	 */
	public void start() {
	...
	/**
	 * The run method for the audio player thread. Simply calls play() on the player
	 * to play the entire stream.
	 */
	public void run() {
		if (player != null) {
			try {
